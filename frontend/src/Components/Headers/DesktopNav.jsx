  ...
  return (
    <nav className="hidden md:flex items-center gap-8 relative font-medium">
      ...
      <div
        className="relative"
        onMouseEnter={() =>
          handleMouseEnter(setShopOpen, shopTimer, setShopTimer)
        }
      ...
      >
        ...
        {shopOpen && (
          <div
            className="absolute left-1/2 -translate-x-1/2 mt-2 bg-white shadow-lg rounded-xl p-6
                       flex flex-wrap gap-10 min-w-[900px] max-w-[90vw] overflow-x-auto
          ...
          >
            {categories.length > 0 ? (
              categories
                ...
                .map((cat, catIndex) => (
                  <React.Fragment key={cat.slug || cat._id || catIndex}>
                    {cat.subcategories?.map((sub, subIndex) => (
                      <div
                        key={sub.slug || sub._id || subIndex}
                        className="min-w-[180px] flex-shrink-0"
                      >
                        <h4 className="text-gray-700 font-semibold mb-2  pb-1">
                          {sub.name}
                        </h4>
      ...
      <div
        className="relative"
        onMouseEnter={() =>
          handleMouseEnter(setRecipesOpen, recipesTimer, setRecipesTimer)
        }
      ...
      >
        ...
        {recipesOpen && recipeItems.length > 0 && (
          <div
            ...
            onMouseLeave={() =>
              handleMouseLeave(setRecipesOpen, setRecipesTimer)
            }
          >
            {recipeItems.map((sub) => (
              <Link
      ...
      <div
        className="relative"
        onMouseEnter={() =>
          handleMouseEnter(setAboutOpen, aboutTimer, setAboutTimer)
        }
