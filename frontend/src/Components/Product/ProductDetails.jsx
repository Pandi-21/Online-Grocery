...
export default function ProductDetails() {
  ...
  const { category, subcategory, item, productSlug } = useParams();
  ...
  `/products/${category}/${subcategory}/${item || ""}/${productSlug}`
);
    ...
    fetchProduct();
  }, [category, subcategory, item, productSlug, navigate]);

  if (loading) return <h2 className="p-8">Loading product...</h2>;
  if (!product) return null; // will redirect to 404
  ...
  const handleBuyNow = () => {
    ...
    toast.success(`${product.name} added & redirecting to checkout`, { id: "basket" });
    navigate("/cart");
  };
